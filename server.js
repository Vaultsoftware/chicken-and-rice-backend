...
try { await initFirebase(); console.log("✅ Firebase ready"); }
catch (e) { console.error("❌ Firebase init failed:", e?.message || e); process.exit(1); }
...
const allowedOrigins = [
  ...
  /\.vercel\.app$/i, // allow previews/custom vercel subdomains
  "http://localhost:3000",
  "http://127.0.0.1:3000",
];
app.use(cors({
  origin(origin, cb) {
    ...
    if (allowedOrigins.includes(origin) || allowedOrigins.some((o) => o instanceof RegExp && o.test(origin))) return cb(null, true);
...
// Optional storage readiness guard
app.use((req, res, next) => {
  try { getBucket(); return next(); }
  catch { return res.status(503).json({ error: "storage not ready" }); }
});
...
app.get("/healthz", (_req, res) => {
  let bucket = null;
  try { bucket = getBucket()?.name || null; } catch {}
  res.json({ ok: !!(process.env.MONGO_URI && process.env.JWT_SECRET), mongoUriConfigured: !!process.env.MONGO_URI, jwtConfigured: !!process.env.JWT_SECRET, bucket });
});
app.get("/__diag/ping", (_req, res) => { try { res.json({ ok: true, bucket: getBucket()?.name || null }); } catch { res.json({ ok: false }); } });
...
app.post("/__diag/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    let buffer = req.file.buffer; if (!buffer && req.file.path) buffer = fs.readFileSync(req.file.path);
...
app.post("/__diag/multipart", upload.single("imageFile"), async (req, res) => {
  // Echo back exactly what Multer sees (for UI debugging)
  res.json({
    fields: req.body || {},
    file: req.file ? {
